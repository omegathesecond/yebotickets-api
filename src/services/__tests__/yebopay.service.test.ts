 ...
 *
 * Why the host gets its own test: the gateway is `api.yebopay.APP`. The
 * similarly-named `.com` domain is an unrelated third-party site (Apache/PHP)
 * that Omevision does not control. Defaulting to the wrong TLD would ship our
 * `x-api-key` and every charge body to a stranger and no purchase would ever
 * settle — a silent, total payment outage that no other test in this repo
 * would catch, because every other suite mocks the client away.
 */
...
describe('yebopay.service — gateway host', () => {
  it('defaults to the REAL gateway https://api.yebopay.app (never the third-party lookalike)', async () => {
    ...
    // Exact match: fails loudly if the default ever reverts to the wrong TLD.
