 ...
 *
 * NB the host is yebopay.APP, not .com — `api.yebopay.com` is an unrelated
 * third-party site (Apache/PHP), NOT this gateway. Defaulting to .com would ship
 * our `x-api-key` and every charge request to a host we do not control, and no
 * payment would ever complete. The live domain mappings are the authority:
